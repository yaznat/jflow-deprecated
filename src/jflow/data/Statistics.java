...
class Statistics {
    ...
    private static int indexHelper(int opIndex, int axis, int[] dimensions, int[] strides) {
        ...
        // Create a list of dimensions excluding the axis dimension
        int[] nonAxisDims = new int[3];
        int nonAxisIndex = 0;
        for (int i = 0; i < 4; i++) {
            if (i != axis) {
                nonAxisDims[nonAxisIndex++] = dimensions[i];
            }
        }
        
        // Convert linear index to multi-dimensional indices (excluding axis)
        int remainingIndex = opIndex;
        int[] nonAxisIndices = new int[3];
        for (int i = 0; i < 3; i++) {
            int product = 1;
            for (int j = i + 1; j < 3; j++) {
                product *= nonAxisDims[j];
            }
            nonAxisIndices[i] = remainingIndex / product;
            remainingIndex %= product;
        }
        
        // Map back to 4D indices
        nonAxisIndex = 0;
        for (int i = 0; i < 4; i++) {
            if (i == axis) {
                indices[i] = 0;
            } else {
                indices[i] = nonAxisIndices[nonAxisIndex++];
            }
        }
